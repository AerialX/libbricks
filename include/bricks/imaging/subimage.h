...
namespace Bricks { namespace Imaging {
	...
	public:
		...
		Colour GetPixel(u32 x, u32 y) const { if (x >= width || y >= height) throw InvalidArgumentException(); return image->GetPixel(offsetX + x, offsetY + y); }
		void SetPixel(u32 x, u32 y, const Colour& colour) { if (x >= width || y >= height) throw InvalidArgumentException(); image->SetPixel(offsetX + x, offsetY + y, colour); }
	};
} }
