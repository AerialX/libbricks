...
namespace Bricks { namespace Collections {
	...
	public:
		virtual Pair< TKey, TValue >& GetCurrent() const { if (!first || position == end) Throw(InvalidIteratorException); return static_cast<Pair < TKey, TValue >&>(current = Pair< TKey, TValue >(position)); }
